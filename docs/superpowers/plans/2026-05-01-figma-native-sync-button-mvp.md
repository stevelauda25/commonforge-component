...
- **Build command:** `cd ../.. && pnpm install --frozen-lockfile && pnpm -F pod-test-tokens build && pnpm -F pod-test-ui build && pnpm -F docs build`
...
```bash
pnpm install
pnpm -F pod-test-tokens build
pnpm -F pod-test-ui build
pnpm -F docs build
```
