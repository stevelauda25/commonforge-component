...
centernode/src/utils/podRuntime.js             — sweep for old token names
...
- `pnpm --filter pod-test-tokens build` clean.
- `pnpm --filter pod-test-ui build` clean.
...
- **Centernode and client-test consume `pod-test-ui` via npm.** Per CLAUDE.md, local changes here won't show in deployed centernode until `/publish` runs and bumps centernode's dependency. Out of scope for this PR; `/publish` is a separate user-triggered command.
...
- Publishing new `pod-test-ui` to npm — separate `/publish` flow.
