...
const { REAL_MAP_PINS, REAL_TOTAL_ACTIVE_JOBS }: { REAL_MAP_PINS: any[], REAL_TOTAL_ACTIVE_JOBS: number } = { REAL_MAP_PINS: [], REAL_TOTAL_ACTIVE_JOBS: 0 } as any;
interface RealMapPin { label: string; status: "staffed" | "in-progress"; job: string; id: string; crew: number; lat: number; lng: number }
...
const BUDGET_KPIS: typeof BUDGET_KPIS_SAMPLE = REAL_BUDGET_KPIS.length > 0 ? (REAL_BUDGET_KPIS as typeof BUDGET_KPIS_SAMPLE) : BUDGET_KPIS_SAMPLE
...
const BUDGET_LEGEND: typeof BUDGET_LEGEND_SAMPLE = REAL_BUDGET.length > 0 ? (REAL_BUDGET as typeof BUDGET_LEGEND_SAMPLE) : BUDGET_LEGEND_SAMPLE
...
const BUDGET_UTILIZATION_ROWS =
  REAL_BUDGET_UTILIZATION.length > 0 ? (REAL_BUDGET_UTILIZATION as typeof BUDGET_UTILIZATION_ROWS_SAMPLE) : BUDGET_UTILIZATION_ROWS_SAMPLE
...
const MAP_PINS: RealMapPin[] = REAL_MAP_PINS.length > 0 ? (REAL_MAP_PINS as RealMapPin[]) : (MAP_PINS_SAMPLE as RealMapPin[])
